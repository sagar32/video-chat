...
techhive.controller('home', ['$scope', '$window', '$state', '$timeout', 'Room', '$http', 'socket', function ($scope, $window, $state, $timeout, Room, $http, socket) {
    ...
    $scope.onLoginSuccess = function () {
        ...
        $http.get("/allUserRightSideList").success(function (allUsersList) {
            ...
            if ($scope.allUserList.length > 0)
                $scope.connectUserRoom($scope.allUserList[0]);
        });
    };
    $scope.onLoginSuccess();
    ...
    $scope.videoCall = function () {
        $scope.videocallAvailable = false;
        Room.init($scope.activeUsername.username, $scope.openRoom, 'v');
        Room.calling($scope.openRoom, $scope.chatWith.username, $scope.activeUsername.username, 'v');
    }
    socket.on('calling', function (room, rUser, cUser, callType) {
        var audioOrVideo=callType=='v'?'Video':'Voice';
        if ($scope.activeUsername.username == rUser) {
            if (confirm(cUser + " Calling..."+"\n its "+audioOrVideo+" call")) {
                $scope.videocallAvailable = false;
                Room.init(rUser, room, callType);
    ...
    $scope.voiceCall = function () {
        $scope.videocallAvailable = false;
        Room.init($scope.activeUsername.username, $scope.openRoom, 'a');
        Room.calling($scope.openRoom, $scope.chatWith.username, $scope.activeUsername.username, 'a');
    }
    // video call end
...
techhive.service('Room', ['$rootScope', 'socket', function ($rootScope, socket) {
    ...
    var room = {
        ...
        init: function (userName, roomName, isVideoCall) {
            ...
            this.socket.on('userlist', function (list) {
                ...
                if (isVideoCall == 'v') {
                    ...
                    room.initStream(room.cons.audioOnly);
                }
            ...
            this.socket.on('disconnect', function () {
                for (var user in room.users)
                    room.userDel(user);
                if (room.localStream)
                    room.localStream.getTracks().forEach(function (track) {
                        track.stop();
                        //location.reload();
                    });
            });
        ...
        calling: function (roomStr, rUser, cUser, callType) {
            room.socket.emit('calling', roomStr, rUser, cUser, callType);
        },

        endCall: function () {
            console.log(room.localStream);
            room.localStream.getTracks().forEach(function (track) {
                track.stop();
                location.reload();
